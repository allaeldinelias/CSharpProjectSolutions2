...
var OlympicRouter = Backbone.Router.extend({
	initialize: function(){
	    this.getArchetypes();
	    document.addEventListener("deviceready", jQuery.proxy(this.deviceReady, this));
	},
	getArchetypes: function(){
		jQuery.ajax({url: "Archetype.cshtml"}).
		done(jQuery.proxy(this.renderMenu, this)).
		fail(function(err){console.log(err);});
	},
	deviceReady: function () {
	    jQuery("#status").html("device ready");
	},
	aArchetypes: [],
		...
		for(var n = 0; n < aAttributes.length; n++){
			...
			switch(oAttribute.formType){
				case 'number':
					...
					break;
				case 'date':
					oAttribute.SQLType = 'DATETIME';
					break;
				default:
					oAttribute.SQLType = 'NVARCHAR(45)';
					break;
	...
	{
		jQuery.ajax({
		    url:"InspectionObject.cshtml?archetype=" + this.sCurrentArchetype,
			dataType:"json"
		}).
	...
	showObjects: function(aObjects){
		...
		jQuery.ajax({
			url:"Archetype.cshtml?archetype=" + this.sCurrentArchetype,
			dataType:"json"
		}).
